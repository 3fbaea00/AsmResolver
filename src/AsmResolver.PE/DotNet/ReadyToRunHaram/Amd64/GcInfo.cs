...
namespace AsmResolver.PE.DotNet.ReadyToRun.Amd64
{
    public class GcInfo : BaseGcInfo
    {
        ...
        private void ParseHeaderFlags(NativeReader imageReader, ref int bitOffset)
        {
            ...
            if (_slimHeader)
            {
                headerFlags = imageReader.ReadBits(1, ref bitOffset) == 1 ? GcInfoHeaderFlags.HasStackBaseRegister : 0;
            }
            else
            {
            ...
            _hasSecurityObject = (headerFlags & GcInfoHeaderFlags.HasSecurityObject) != 0;
            _hasGSCookie = (headerFlags & GcInfoHeaderFlags.HasGSCookie) != 0;
            _hasPSPSym = (headerFlags & GcInfoHeaderFlags.HasPspSym) != 0;
            ...
            _wantsReportOnlyLeaf = ((headerFlags & GcInfoHeaderFlags.WantsReportOnlyLeaf) != 0);
        }
