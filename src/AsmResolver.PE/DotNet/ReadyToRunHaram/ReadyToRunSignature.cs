...
namespace AsmResolver.PE.DotNet.ReadyToRun
{
    ...
    public class R2RSignatureDecoder<TType, TMethod, TGenericContext>
    {
        ...
        public uint ReadToken()
        {
            ...
            {
                case 0:
                    type = TableIndex.TypeDef;
                    break;

                case 1:
                    type = TableIndex.TypeRef;
                    break;

                case 2:
                    type = TableIndex.TypeSpec;
                    break;

                case 3:
                    type = TableIndex.BaseType;
                    break;
    ...
    public class SignatureDecoder : R2RSignatureDecoder<string, string, TextSignatureDecoderContext>
    {
        ...
        private uint ParseMethodDefToken(StringBuilder builder, string owningTypeOverride)
        {
            ...
            uint methodDefToken = ReadUIntAndEmitInlineSignatureBinary(signaturePrefixBuilder) | (uint)TableIndex.Method;
        ...
        {
            StringBuilder signaturePrefixBuilder = new StringBuilder();
            uint methodRefToken = ReadUIntAndEmitInlineSignatureBinary(signaturePrefixBuilder) | (uint)TableIndex.MemberRef;
            builder.Append(MetadataNameFormatter.FormatHandle(
                _metadataReader,
                MetadataTokens.Handle((int)methodRefToken),
        ...
        private void ParseField(StringBuilder builder)
        {
            ...
            {
                fieldToken = ReadUIntAndEmitInlineSignatureBinary(signaturePrefixBuilder) | (uint)TableIndex.Field;
            }
            builder.Append(MetadataNameFormatter.FormatHandle(
                _metadataReader,
